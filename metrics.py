...
class MetricsCalculator(PortfolioOptimizer):
    ...
    def portfolioReturnsGraph(self):
        ...
        returns_df = pd.DataFrame(
            {
                ...
                "S&P500 Cumulative Return (%)": cumulative_returns_b.values,
        ...
        fig = px.line(
            returns_df,
            x="Date",
            y=["Portfolio Cumulative Return (%)", "S&P500 Cumulative Return (%)"],
        ...
        st.markdown(f'**S&P500 Returns**: {round(cumulative_returns_b.values[-1], 2)}% ')
        st.plotly_chart(fig)
